...
export const getJobApplicationsByStatusAndJobSeekerService = async (req: Request, status: string): Promise<any> => {
    try {
        const decodedToken = getToken(req);
        if (!decodedToken) {
            throw new ErrorHandler({
        ...
        const jobApplications = await jobApplicationDao.getJobApplicationsByJobSeekerIdAndStatusDao(jobSeeker.id, status);
        return {
            success: true,
            message: 'Job applications retrieved successfully.',
            data: jobApplications,
